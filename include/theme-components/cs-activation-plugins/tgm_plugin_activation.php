	...
	class TGM_Plugin_Activation {
		...
		public function init() {
			...
			$this->strings = array(
				...
				'installing'                      => __( 'Installing Plugin: %s', 'Awaken' ),
				'oops'                            => __( 'Something went wrong.', 'Awaken' ),
				...
				'dashboard'                       => __( 'Return to the dashboard', 'Awaken' ),
				'plugin_activated'                => __( 'Plugin activated successfully.', 'Awaken' ),
				'activated_successfully'          => __( 'The following plugin was activated successfully:', 'Awaken' ),
				'complete'                        => __( 'All plugins installed and activated successfully. %1$s', 'Awaken' ),
				'dismiss'                         => __( 'Dismiss this notice', 'Awaken' ),
				'contact_admin'                   => __( 'Please contact the administrator of this site for help.', 'Awaken' ),
			);
		...
		protected function do_plugin_install() {
			...
			if ( isset( $this->plugins[ $slug ], $_GET['tgmpa-install'] ) && 'install-plugin' === $_GET['tgmpa-install'] ) {
				...
				if ( true === $complete ) {
					echo '<p>', sprintf( esc_html( $this->strings['complete'] ), '<a href="' . esc_url( self_admin_url() ) . '">' . esc_html__( 'Return to the Dashboard', 'Awaken' ) . '</a>' ), '</p>';
			...
			if ( ! empty( $desired_slug ) ) {
				...
				if ( ! empty( $subdir_name ) && $subdir_name !== $desired_slug ) {
					...
					if ( true === $GLOBALS['wp_filesystem']->move( $from, $to ) ) {
						...
						return new WP_Error( 'rename_failed', esc_html__( 'The remote plugin package is does not contain a folder with the desired slug and renaming did not work.', 'Awaken' ) . ' ' . esc_html__( 'Please contact the plugin provider and ask them to package their plugin according to the WordPress guidelines.', 'Awaken' ), array( 'found' => $subdir_name, 'expected' => $desired_slug ) );

					}
				} elseif ( empty( $subdir_name ) ) {
					return new WP_Error( 'packaged_wrong', esc_html__( 'The remote plugin package consists of more than one file, but the files are not packaged in a folder.', 'Awaken' ) . ' ' . esc_html__( 'Please contact the plugin provider and ask them to package their plugin according to the WordPress guidelines.', 'Awaken' ), array( 'found' => $subdir_name, 'expected' => $desired_slug ) );
				}
			}
		...
		protected function activate_single_plugin( $file_path, $slug, $automatic = false ) {
			...
			if ( ! is_plugin_active( $file_path ) ) {
				...
				echo '<div id="message" class="error"><p>',
					sprintf(
						esc_html__( 'No action taken. Plugin %1$s was already active.', 'Awaken' ),
		...
		public function notices() {
			...
			if ( ! empty( $message ) ) {
				...
				foreach ( $message as $type => $plugin_group ) {
					...
					$imploded       = empty( $linked_plugins ) ? $last_plugin : ( implode( ', ', $linked_plugins ) . ' ' . esc_html_x( 'and', 'plugin A *and* plugin B', 'Awaken' ) . ' ' . $last_plugin );
	...
	class TGMPA_List_Table extends WP_List_Table {
			...
			foreach ( $this->tgmpa->plugins as $slug => $plugin ) {
				...
				if ( 'repo' !== $plugin['source'] && preg_match( TGM_Plugin_Activation::WP_REPO_REGEX, $plugin['source'] ) !== 1 ) {
					if ( preg_match( TGM_Plugin_Activation::IS_URL_REGEX, $plugin['source'] ) ) {
						...
						$table_data[ $i ]['source'] = __( 'External Source', 'Awaken' );

					} else {
						// The plugin is pre-packaged with the theme.
						$table_data[ $i ]['source'] = __( 'Pre-Packaged', 'Awaken' );
					}
				}
				// The plugin is from the WordPress repository.
				else {
					$table_data[ $i ]['source'] = __( 'WordPress Repository', 'Awaken' );
				}

				if ( isset( $plugin['required'] ) && $plugin['required'] ) {
					$table_data[ $i ]['type'] = __( 'Required', 'Awaken' );
				} else {
					$table_data[ $i ]['type'] = __( 'Recommended', 'Awaken' );
				}

				if ( ! isset( $installed_plugins[ $plugin['file_path'] ] ) ) {
					$table_data[ $i ]['status'] = sprintf( '%1$s', __( 'Not Installed', 'Awaken' ) );
				} elseif ( is_plugin_inactive( $plugin['file_path'] ) ) {
					$table_data[ $i ]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', 'Awaken' ) );
				}
		...
		public function column_plugin( $item ) {
			...
			if ( ! is_plugin_active( $item['file_path'] ) && ! isset( $installed_plugins[ $item['file_path'] ] ) ) {
				...
				$actions = array(
					'install' => sprintf(
						'<a href="%1$s">' . esc_html_x( 'Install %2$s', '%2$s = plugin name in screen reader markup', 'Awaken' ) . '</a>',
			...
			elseif ( is_plugin_inactive( $item['file_path'] ) ) {
				...
				$actions = array(
					'activate' => sprintf(
						'<a href="%1$s">' . esc_html_x( 'Activate %2$s', '%2$s = plugin name in screen reader markup', 'Awaken' ) . '</a>',
		...
		public function no_items() {
			...
			printf( wp_kses_post( __( 'No plugins to install or activate. <a href="%1$s">Return to the Dashboard</a>', 'Awaken' ) ), esc_url( self_admin_url() ) );
			echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
		...
		public function get_columns() {
			...
			$columns = array(
				...
				'plugin' => __( 'Plugin', 'Awaken' ),
				'source' => __( 'Source', 'Awaken' ),
				'type'   => __( 'Type', 'Awaken' ),
				'status' => __( 'Status', 'Awaken' ),
			);
		...
		public function get_bulk_actions() {
			...
			$actions = array(
				'tgmpa-bulk-install'  => __( 'Install', 'Awaken' ),
				'tgmpa-bulk-activate' => __( 'Activate', 'Awaken' ),
			);
		...
		public function process_bulk_actions() {
			...
			if ( 'tgmpa-bulk-install' === $this->current_action() ) {
				...
				if ( empty( $plugins_to_install ) ) {
					echo '<div id="message" class="error"><p>', esc_html__( 'No plugins are available to be installed at this time.', 'Awaken' ), '</p></div>';
					return false;
				}
			...
			if ( 'tgmpa-bulk-activate' === $this->current_action() ) {
				...
				if ( empty( $plugins_to_activate ) ) {
					echo '<div id="message" class="error"><p>', esc_html__( 'No plugins are available to be activated at this time.', 'Awaken' ), '</p></div>';
					return false;
				}
				...
				if ( is_wp_error( $activate ) ) {
					...
					$imploded     = empty( $plugin_names ) ? $last_plugin : ( implode( ', ', $plugin_names ) . ' ' . esc_html_x( 'and', 'plugin A *and* plugin B', 'Awaken' ) . ' ' . $last_plugin );

					printf(
						'<div id="message" class="updated"><p>%1$s %2$s.</p></div>',
						esc_html( _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, 'Awaken' ) ),
			...
			if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
				...
				class TGM_Bulk_Installer extends WP_Upgrader {
					...
					public function install_strings() {
						...
						$this->strings['no_package']          = __( 'Install package not available.', 'Awaken' );
						$this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>&#8230;', 'Awaken' );
						$this->strings['unpack_package']      = __( 'Unpacking the package&#8230;', 'Awaken' );
						$this->strings['installing_package']  = __( 'Installing the plugin&#8230;', 'Awaken' );
						$this->strings['process_failed']      = __( 'Plugin install failed.', 'Awaken' );
						$this->strings['process_success']     = __( 'Plugin installed successfully.', 'Awaken' );
					...
					public function activate_strings() {
						...
						$this->strings['activation_failed']  = __( 'Plugin activation failed.', 'Awaken' );
						$this->strings['activation_success'] = __( 'Plugin activated successfully.', 'Awaken' );
			...
			if ( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
				...
				class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
					...
					public function add_strings() {
						...
						$this->upgrader->strings['skin_update_failed_error'] = __( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', 'Awaken' );
						$this->upgrader->strings['skin_update_failed']       = __( 'The installation of %1$s failed.', 'Awaken' );

						// Automatic activation strings.
						if ( $this->tgmpa->is_automatic ) {
							$this->upgrader->strings['skin_upgrade_start']        = __( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', 'Awaken' );
							$this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', 'Awaken' ) . ' <a href="#" class="hide-if-no-js" onclick="%2$s"><span>' . esc_html__( 'Show Details', 'Awaken' ) . '</span><span class="hidden">' . esc_html__( 'Hide Details', 'Awaken' ) . '</span>.</a>';
							$this->upgrader->strings['skin_upgrade_end']          = __( 'All installations and activations have been completed.', 'Awaken' );
							$this->upgrader->strings['skin_before_update_header'] = __( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', 'Awaken' );
						}
						// Default installation strings.
						else {
							$this->upgrader->strings['skin_upgrade_start']        = __( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', 'Awaken' );
							$this->upgrader->strings['skin_update_successful']    = esc_html__( '%1$s installed successfully.', 'Awaken' ) . ' <a href="#" class="hide-if-no-js" onclick="%2$s"><span>' . esc_html__( 'Show Details', 'Awaken' ) . '</span><span class="hidden">' . esc_html__( 'Hide Details', 'Awaken' ) . '</span>.</a>';
							$this->upgrader->strings['skin_upgrade_end']          = __( 'All installations have been completed.', 'Awaken' );
							$this->upgrader->strings['skin_before_update_header'] = __( 'Installing Plugin %1$s (%2$d/%3$d)', 'Awaken' );
						}
					...
					public function bulk_footer() {
						...
						if ( true === $complete ) {
							echo '<p>', sprintf( esc_html( $this->tgmpa->strings['complete'] ), '<a href="' . esc_url( self_admin_url() ) . '">' . esc_html__( 'Return to the Dashboard', 'Awaken' ) . '</a>' ), '</p>';
							echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
						}
